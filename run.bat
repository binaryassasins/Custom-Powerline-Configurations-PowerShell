...
powershell -Command "& {
		...
		if (Test-Path $profilePath) {
			Copy-Item -Path $profilePath -Destination ~\Documents\PowerShell;
		       	Write-Host 'Profile replaced successfully.'	
		} else {
			Write-Host 'Please ensure that you have installed "Windows Terminal" from Microsoft Store (https://apps.microsoft.com/detail/9n0dx20hk701?hl=en-US&gl=US)';
		}
